...
INSERT INTO tbl_iolist(io_date, io_time, io_pname, io_dname ,io_dceo, io_div, io_quan, io_price, io_oprice)
VAlUES
...
SELECT*
FROM tbl_iolist LIMIT 3 OFFSET 10;

/*
2023년 02-01 이전의 데이터만 보여주기
io_date 칼럼은 문자열형 데이터 이지만
SQL에서는 가능하다 단 자리수가 같아야함
*/
SELECT*
FROM tbl_iolist
...
-- 시작이상 종료이하의 범위는 BETWEEN을 사용하여 검사한다
SELECT*
FROM tbl_iolist
...
SELECT*
FROM tbl_iolist
WHERE io_date = "2023-02-25" OR io_date = "2023-02-10" OR io_date = "2023-03-07";

-- 거래구분(io_div)이 2이고
-- 거래수량이(io_quan) 50개 이하인 데이터만 SELECTION
...
-- 계산식의 결과 확인
SELECT io_quan * io_iprice, io_quan * io_oprice
FROM tbl_iolist;

-- 계산식에 별명(Alias) 붙이기
SELECT
	io_quan * io_iprise AS 매입합계,
	io_quan * io_oprise AS 매출합계
FROM tbl_iolist;

/*
상품명(io_pname)이 같은 값끼리 GROUP을 만들어가
그리고 같은 GROUP 내에서 합산하여 결과를 보여주기
*/
SELECT
	io_pname AS 상품명,
	io_quan * io_iprise AS 매입합계,
...
FROM tbl_iolist
GROUP BY io_pname;
-- 상품별로 매입 매출이 각각 몇번씩 이루어졋나?
-- 상품명으로 GROUP을 설정하고
-- 같은상품중에서 거래구분으로 분리하여 GROUP 설정
-- 그리고 그 GROUP에서 개수를 세어서 보여라
SELECT io_pname AS 상품명,
	io_div AS 거래구분,
    COUNT(io_pname) AS 횟수
FROM tbl_iolist
