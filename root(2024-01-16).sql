/*
-- DB Handling
SQL 명령을 통해서 DBMS의 storage에 저장된 데이터에 접근하여 
여러가지 연산을 수행하는 것
저장소(Scheme, Schema 스키마)를 설계, 생성
저장소를 기준으로 데이터를 추가, 수정, 삭제, 조회
DDL(Data Definition Lang. 데이터 정의의)
저장소를 생성, 삭제, 변경하는 명령어 체계
...
PRIMARY KEY
NOT NULL 이면서 UNIQUE한 성질 부여하기
절대 공백이 없고, 중복된 데이터가 없어야 한다
==객체무결성 설정

NOT NULL
절대 공백인 데이터가 없어야 한다
불필요한 공백인 데이터가 저장되어 있으면 그 데이터는 공간을 낭비할 뿐이다

UNIQUE
중복된 데이터가 없다
*/
-- 데이터를 CRUD를 실행하자
-- 새로운(Table에 없는데이터)를 Table에 추가하기
-- INSERT [table] INTO([columns]) VALUES (값들)
...
-- 필수항목(PK)인 st_num의 데이터가 준비되지 않아서 
-- 데이터추가실패
INSERT INTO
tbl_student(st_name,st_dept)
VALUES('개원훈','기계공학과');

-- 필수항목(NOT NULL)인 st_name의 데이터가 준비되지 않아서 
-- 데이터추가실패
INSERT INTO
tbl_student(st_num,st_dept)
VALUES('S0003','기계공학과');

-- 필수항목(NOT NULL)인 st_dept의 데이터가 준비되지 않아서 
-- 데이터추가실패
INSERT INTO
...
-- st_num, st_name, st_dept 칼럼의 데이터만 선택적으로 보고싶다
...
-- SELECTION
-- 어떤 조건을 부여하여 조건에 맞는 데이터들만 확인하고싶을 때
...
-- st_name에 조건을 부여하여 UPDATE를 실행하고 있다
-- st_name 칼럼은 데이터가 중복가능(동명이인)
-- 때문에 아래와 같은 코드도 문제를 일으킬 수 있다
...
-- UPDATE 순서
-- 1. UPDATE 할 조건의 데이터를 SELCET한다
-- 2. SELECT 된 데이터 목록중에서 Update 할 데이터의
-- PK값을 확인한다
-- 3. PK 칼럼을 기준으로 Update를 실행한다
SELECT * FROM tbl_student
WHERE st_name = '강이찬';
UPDATE tbl_student
SET st_grade = '3'
...
/*
DML(Date Manifulation Lang. 데이터 관리어,데이터관리어)
이미 존재하는 DB, Table에 새로운 데이터를 
추가(Create, INWERT INTO)
...
CREATE TABLE tbl_professor (
	pr_num	VARCHAR(4)	PRIMARY KEY,
	pr_name	VARCHAR(10)	NOT NULL,
	pr_tel	VARCHAR(13) NOT NULL
